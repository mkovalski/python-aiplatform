...
from google.cloud.aiplatform.training_utils.debugging import initialize
...
def test_start_debugger_no_plugins():
    with mock.patch.object(
        initialize, '_run_app_thread', return_value=None
    ) as mock_app_thread:
        initialize.start_debugger(plugins=[])
        assert mock_app_thread.call_count == 0

def test_start_debugger_bad_plugin():
    ...
    with mock.patch.object(
        ...
        with mock.patch.dict(initialize.MAP_TO_PLUGIN, mock_map):
            initialize.start_debugger(plugins=['plugin1', 'plugin2'])
        mock_app_thread.assert_called_with([mock_plugin], 6010)

def test_start_debugger_duplicate_plugins():
    ...
    with mock.patch.object(
        ...
        with mock.patch.dict(initialize.MAP_TO_PLUGIN, mock_map):
            initialize.start_debugger(plugins=['plugin1', 'plugin1'])
        mock_app_thread.assert_called_with([mock_plugin], 6010)

def test_start_debugger_fail_initiliaze_plugins():
